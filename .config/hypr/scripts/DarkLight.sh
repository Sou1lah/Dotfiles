...
set_waybar_style() {
    ...
    case "$theme" in
        Dark)
            ln -sf "$HOME/.config/waybar/style/main.css" "$waybar_style_link"
            ;;
        Light)
            ln -sf "$HOME/.config/waybar/style/light.css" "$waybar_style_link"
            ;;
        *)
            echo "No matching Waybar style for theme: $theme"
            ;;
    esac
}
