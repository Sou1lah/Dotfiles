...
function _G.open_folder()
  ...
  if ok and telescope.extensions and telescope.extensions.file_browser then
    telescope.extensions.file_browser.file_browser({
      prompt_title = "Ranger-like File Browser",
      cwd = vim.loop.cwd(),
      hidden = true,
      respect_gitignore = false,
      select_buffer = true,
      files = true,   -- show files and folders for navigation
      browser = true, -- enable browser mode for navigation
      attach_mappings = function(prompt_bufnr, map)
        local actions = require("telescope.actions")
        local action_state = require("telescope.actions.state")
        -- <C-y> to open Vim in the selected directory
        local function open_in_dir()
          ...
          if dir and vim.fn.isdirectory(dir) == 1 then
            ...
            vim.cmd("Dashboard")
            require("nvim-tree.api").tree.change_root(abs_path)
            _G.save_recent_project(abs_path)
          else
            vim.notify("Please select a directory!", vim.log.levels.WARN)
          end
        end
        map("i", "<C-y>", open_in_dir)
        map("n", "<C-y>", open_in_dir)
        return true
      end,
    })
    return
...
function _G.show_keybinds_telescope()
  local keybinds = {
    ...
    { "Plugins", {
      ...
      { "<leader>lg",  "Lazygit (auto-close)" },
      { "<leader>ghg", "Ghgrab (auto-close)" },
